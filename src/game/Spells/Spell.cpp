...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (m_targets.m_targetMask == TARGET_FLAG_DEST_LOCATION)
    {
        ...
        if (!IsIgnoreLosSpell(m_spellInfo))
            if (!m_trueCaster->IsWithinLOS(m_targets.m_destPos.x, m_targets.m_destPos.y, m_targets.m_destPos.z + 1.f, false, true))
                return SPELL_FAILED_LINE_OF_SIGHT;
    }
    ...
    if (m_targets.m_targetMask == TARGET_FLAG_SOURCE_LOCATION)
    {
        ...
        if (minRange && dist < minRange * minRange)
            return SPELL_FAILED_TOO_CLOSE;
        if (!IsIgnoreLosSpell(m_spellInfo))
            if (!m_trueCaster->IsWithinLOS(m_targets.m_srcPos.x, m_targets.m_srcPos.y, m_targets.m_srcPos.z + 1.f, false, true))
                return SPELL_FAILED_LINE_OF_SIGHT;
    }
